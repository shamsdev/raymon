...
class BufferHandler {
    ...
    _checkHeader() {
        ...
        if (this.buffer.length >= Packet.headerLength) {
            this.header = BufferHandler.getHeader(this.buffer, Packet.headerLength);
            this.buffer = this.buffer.slice(Packet.headerLength);
...
BufferHandler.onReceiveDatagramPacket = function (dgram, callback) {
    const header = this.getHeader(dgram, Packet.headerLength);
    dgram = dgram.slice(Packet.headerLength);
